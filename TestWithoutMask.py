...
def predict(args):
    ...
    for id in xrange(70):
        ...
        #  load data
        img_test, img_test_header, mask, mask_header = load(args.data + str(id) + '.nii')
        img_test -= args.mean

        # change format to all background 
        mask[mask==2]=1
        mask[mask==0]=1

        mask = ndimage.binary_dilation(mask, iterations=1).astype(mask.dtype)
        index = np.where(mask==1)
        # mini should be (0 0 0)
        mini = np.min(index, axis = -1)
        # maxi should be size of shape (x, y, z-1)
